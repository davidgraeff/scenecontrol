...
public Q_SLOTS:
     ...
     */
    QList<QVariantMap> requestAllOfType(SceneDocument::TypeEnum type, const QVariantMap& filter = QVariantMap());

    /// Remove a document
    void removeDocument(SceneDocument::TypeEnum type, const QString& docid);
     ...
     */
    int changeDocumentsValue(SceneDocument::TypeEnum type, const QVariantMap& filter, const QVariantMap& value);
    
    /**
      * Return true if document with type and id is already stored
      */
    bool contains(const SceneDocument &doc);
