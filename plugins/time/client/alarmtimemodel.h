...
class AlarmTimeModel : public ClientModel
{
    Q_OBJECT
public:
    AlarmTimeModel (QObject* parent = 0);
    ~AlarmTimeModel();
...
//	virtual bool setData(const QModelIndex &index, const QVariant &value, int role = Qt::EditRole);
    ...
    void focusAsSoonAsAvailable(const QString& id) {
        m_focusid=id;
    }
public Q_SLOTS:
    virtual void clear();
...
private:
    struct AlarmItem {
        AbstractServiceProvider* item;
	Collection* parent;
	AlarmItem(AbstractServiceProvider* item, Collection* p) : item(item), parent(p) {}
    };
