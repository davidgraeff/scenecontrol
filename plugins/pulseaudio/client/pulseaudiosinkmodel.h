...
class PulseAudioSinkModel : public ClientModel
{
    Q_OBJECT
public:
	struct PulseAudioSinkItem {
		QString name;
		double volume;
		bool mute;
		PulseAudioSinkItem(const QString& name, double volume, bool mute) {
			this->mute = mute;
			this->name = name;
			this->volume = volume;
		}
    };
    PulseAudioSinkModel (QObject* parent=0);
...
private:
	QList<PulseAudioSinkItem> m_items;
public Q_SLOTS:
	virtual void clear();
