...
{
    m_statetracker = new CinemaStateTracker();
...
CinemaController::~CinemaController()
{
    m_xbmcClient->SendNOTIFICATION(tr("RoomControServer").toLatin1().constData(),
...
QList<AbstractStateTracker*> CinemaController::getStateTracker()
{
...
void CinemaController::setCommand(int cmd)
{
    switch (cmd) {
    case PlayCmd:
...
void CinemaController::setPosition ( int pos, bool relative )
{
  Q_UNUSED(pos);
  Q_UNUSED(relative);
}

void CinemaController::setVolume ( int vol, bool relative )
{
  Q_UNUSED(vol);
  Q_UNUSED(relative);
}
