...
void QObjectHelper::qvariant2qobject(const QVariantMap& variant, QObject* object)
{
	...
	if (type == QVariant::StringList)
        {
            QVariantList list_source = v.toList();
            QStringList list_destination;
            foreach (QVariant variant_source, list_source) {
                list_destination.append(variant_source.toString());
            }
            v.setValue<QStringList>(list_destination);
        }
